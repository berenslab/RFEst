...
class ARD(EmpiricalBayes):
    ...
    def update_C_prior(self, params):
        ...
        rho = params[1]
        theta = params[2:]
        C, C_inv = sparsity_kernel(theta, jnp.product(self.dims))
        C *= rho
        C_inv /= rho
...
class ARDFixedPoint:
    ...
    def update_params(self, params, C_post, m_post):
        ...
        theta = (self.n_features - theta * jnp.trace(C_post)) / m_post ** 2

        upper = jnp.sum(self.YtY - 2 * self.XtY * m_post + m_post.T @ self.XtX @ m_post)
        lower = self.n_features - jnp.sum(1 - theta * jnp.diag(C_post))
        sigma = upper / lower

        return jnp.hstack([sigma, theta])

    def update_C_prior(self, params):

        theta = params[1:]

        C_prior = jnp.identity(self.n_features) * 1 / theta
        C_prior_inv = jnp.identity(self.n_features) * theta
    ...
    def fit(self, p0, num_iters=100, threshold=1e-6, MAXALPHA=1e6, verbose=True):
        ...
        i = 0
        for i in jnp.arange(1, num_iters + 1):
            ...
            dparams = jnp.linalg.norm(params - params0)
    ...
    def measure_prediction_performance(self, X_test, y_test):
        ...
        wSTA_test = jnp.linalg.solve(X_test.T @ X_test, X_test.T @ y_test)
