            ...
            if verbose:
                if itr % verbose == 0:
                    ...
                    if len(self.cost) >= 10 and (np.abs(np.diff(self.cost[-10:])) < 1e-7).all():
                        ...
                        print('Stop: cost has been changing so small in the last {0} check points. Final cost = {1:.03f}, total time elapsed = {2:.03f} s'.format(tolerance, self.cost[-1], total_time_elapsed))
                        break
        else:
            if verbose:
                total_time_elapsed = time.time() - time_start

                print('Stop: reached {0} iterations. Final cost = {1:.03f}, total time elapsed = {1:.03f} s'.format(num_iters, self.cost[-1], total_time_elapsed))
