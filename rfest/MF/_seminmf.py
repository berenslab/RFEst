...
class semiNMF:
    ...
    def __init__(self, V, k=2, init_method="random", random_seed=2046, **kwargs):
        ...
        self.W, self.H = initialize_factors(
            V, k, method=init_method, random_seed=random_seed
        )
        ...
        for itr in range(num_iters):
            ...
            if verbose:
                if itr % verbose == 0:
                    ...
                    if (
                        len(self.cost) >= 10
                        and (np.abs(np.diff(self.cost[-10:])) < 1e-7).all()
                    ):
            ...
            if verbose:
                ...
                print(
                    "Stop: reached {0} iterations. Final cost = {1:.03f}, total time elapsed = {1:.03f} s".format(
