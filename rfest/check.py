...
def eval_model_score(
    model, X, y, stim=True, history=True, metric="corrcoef", w_type="opt"
):
...
def compute_permutation_test(
    model, X_test, y_test, n_perm=100, history=True, metric="corrcoef", w_type="opt"
):
    ...
    for i in range(n_perm):
        ...
        score_permX[i] = eval_model_score(
            model=model,
            X=permX,
            y=y_test,
            stim=True,
...
def plot_permutation_test(
    ...
    ax.legend(
        handles=[line, bp["boxes"][0], line_q],
        labels=["true X", "perm. X", f"q{q}"],
        loc="lower right",
    )
    ax.plot(-0.3, score_trueX, marker="o", color="lightgray", ms=20, zorder=2, alpha=1)
    ax.plot(
        -0.3, score_trueX, marker="*" if is_greater else "_", color="r", ms=10, zorder=3
    )
...
def significance(model, w_type="opt", show_results=False):
    ...
    for name in model.filter_names:
        W = np.squeeze(
            model.p[w_type][name].T
            @ np.linalg.inv(model.V[w_type][name])
            @ model.p[w_type][name]
        )
...
def residuals_deviance(y, y_pred):
    quo = y / y_pred
    rsd = y - y_pred
    ri = np.sign(rsd) * np.sqrt(
        2 * (y * np.log(quo, out=np.zeros_like(quo), where=(quo != 0)) - rsd)
    )
...
def plot_prediction_testset(model, X_test, y_test, display_window, w_type, ax=None):
    ...
    ax.fill_between(
        tt, y_pred_upper[:length], y_pred_lower[:length], color="red", alpha=0.5
    )
...
def plot_permutation(
    model, kind="train", w_type="opt", q=99, num_repeat=100, ax=None, legend=False
):
    if ax is None:
        _, ax = plt.subplots()
    ...
    score_true = model.compute_score(
        model.y[kind], model.forwardpass(model.p[w_type], kind), model.metric
    )
    ...
    if legend:
        ax.legend(
            handles=[line, bp["boxes"][0], line_q],
            labels=["model pred.", "permuted pred.", f"q{q}"],
        )
    ax.set_title(f"Prediction on permuted {kind} set\n{p_result}")
    ax.set_xlim([-0.4, 0.4])


def plot_permutation_testset(
    model, X_test, y_test, w_type, q=99, num_repeat=100, ax=None
):
    if ax is None:
        _, ax = plt.subplots()
    ...
    score_true = model.compute_score(
        model.y["test"], model.forwardpass(model.p[w_type], "test"), model.metric
    )
    s_perm = []
    for _ in range(num_repeat):
        ...
        s_perm.append(
            model.compute_score(
                model.y["test"],
                model.forwardpass(model.p[w_type], "test"),
                model.metric,
            )
        )
    else:
    ...
    for i, name in enumerate(model.filter_names):
        ...
        vmax = np.max(
            [
                np.abs(w.max()),
                np.abs(w.min()),
                np.abs(w.max() + 2 * w_se.max()),
                np.abs(w.min() - 2 * w_se.min()),
            ]
        )
        tt = (
            np.linspace(
                -model.dims[name][0] - model.shift[name],
                0 - model.shift[name],
                model.dims[name][0],
            )
            * dt
        ...
        filts[i].fill_between(tt, w + 2 * w_se, w - 2 * w_se, color=f"C{i}", alpha=0.5)
...
def plot2d(model, w_type="opt", figsize=None, return_stats=False):
    ...
    for i, name in enumerate(ws):
        if "stimulus" in name:
            ...
            ax_data[name] = {"axes_sRF_min": [], "axes_sRF_max": [], "axes_tRF": []}
            ...
            ref = [sRF[2:-2].max(), sRF[2:-2].min()][
                np.argmax([np.abs(sRF.max()), np.abs(sRF.min())])
            ]
            ...
            if compute_ci:
                ax_sRF_min.fill_between(
                    xrnge, wu[tRF_min], wl[tRF_min], color="C0", alpha=0.5
                )
            ...
            if compute_ci:
                ax_sRF_max.fill_between(
                    xrnge, wu[tRF_max], wl[tRF_max], color="C3", alpha=0.5
                )
            ...
            stats[name]["tRF_activation_diff"].append(
                np.abs(tRF[tRF_max] - tRF[tRF_min])
            )
...
def plot3d(
    model, w_type="opt", contour=0.1, pixel_size=30, figsize=None, return_stats=False
):
    """
    Parameters
    ----------
    ...
    for i, name in enumerate(ws):
        if "stimulus" in name:
            ...
            ax_sRF_min.imshow(
                sRF_min.T, cmap=plt.cm.bwr, vmin=-vmax, vmax=vmax, aspect="auto"
            )
            ...
            ax_data[name]["axes_sRF_max"].append(ax_sRF_max)
            ax_sRF_max.imshow(
                sRF_max.T, cmap=plt.cm.bwr, vmin=-vmax, vmax=vmax, aspect="auto"
            )
    ...
    for name in ws:
        if "stimulus" in name:
            ...
            if cv2 is not None:
                cntrs_size_min = [
                    cv2.contourArea(cntr.astype(np.float32)) * pixel_size**2 / 1000
                    for cntr in cntrs_min
                ]
                axes_sRF_min[i].set_xlabel(
                    f"cntr size = {cntrs_size_min[0]:.03f} 10^3 μm^2"
                )

                cntrs_size_max = [
                    cv2.contourArea(cntr.astype(np.float32)) * pixel_size**2 / 1000
                    for cntr in cntrs_max
                ]
                axes_sRF_max[i].set_xlabel(
                    f"cntr size = {cntrs_size_max[0]:.03f} 10^3 μm^2"
                )
                ...
                stats[name]["sRF_size_diff"].append(
                    np.abs(cntrs_size_min[0] - cntrs_size_max[0])
                )
...
def plot_diagnostics(
    ...
    plot_permutation(
        model, kind="train", w_type=w_type, num_repeat=num_repeat, ax=diagnostics[4]
    )

    if "dev" in model.y:
        diagnostics.append(fig.add_subplot(spec[1, 4]))
        plot_permutation(
            model, kind="dev", w_type=w_type, num_repeat=num_repeat, ax=diagnostics[5]
        )
    # else:
    # diagnostics[-1].axis('off')
    ...
    if "dev" in model.y:
        ...
        y_pred = np.hstack(
            [y_pred_train[-int(length / 2) :], y_pred_dev[: int(length / 2)]]
        )
        ...
        if model.compute_ci:
            ...
            y_pred_lower = np.hstack(
                [
                    y_pred_train_lower[-int(length / 2) :],
                    y_pred_dev_lower[: int(length / 2)],
                ]
            )
            y_pred_upper = np.hstack(
                [
                    y_pred_train_upper[-int(length / 2) :],
                    y_pred_dev_upper[: int(length / 2)],
                ]
            )
    else:
    ...
    if len(y) == length:
        ...
        axes_pred.set_xlabel(
            "(train set)  <-    Time (second)  ->     (dev set)", fontsize=12
        )
    else:
        axes_pred.axvline(
            tt[len(y_train[-int(length / 2) :])], color="black", linestyle="--"
        )
        axes_pred.set_xlabel("Time (second)", fontsize=12)

    axes_pred.set_ylabel("Data")

    metric_train = model.compute_score(
        model.y["train"], model.forwardpass(model.p[w_type], "train"), model.metric
    )
    # metric_train = model.compute_score(model.y['train'], model.y_pred[w_type]['train'], model.metric)
    if "dev" in model.y:
        metric_dev = model.compute_score(
            model.y["dev"], model.forwardpass(model.p[w_type], "dev"), model.metric
        )
        # metric_dev = model.compute_score(model.y['dev'], model.y_pred[w_type]['dev'], model.metric)
        axes_pred.set_title(
            f"{model.metric}(train) = {metric_train:.2f} |  {model.metric}(dev) = {metric_dev:.2f}"
        )
    else:
    ...
    if X_test is not None:
        ...
        plot_prediction_testset(
            model, X_test, y_test, display_window, w_type, ax=axes_pred_test
        )

        diagnostics.append(fig.add_subplot(spec[2, 4]))
        plot_permutation_testset(
            model,
            X_test,
            y_test,
            w_type=w_type,
...
def plot_subunits3d(
    ...
    model_name = (
        str(type(model)).split(".")[-1][:-2] if model_name is None else model_name
    )
    ...
    vmax = np.max(
        [
            np.abs(np.max(sRFs_max)),
            np.abs(np.min(sRFs_max)),
            np.abs(np.max(sRFs_min)),
            np.abs(np.min(sRFs_min)),
        ]
    )
    ...
    for i in range(num_subunits):
        ...
        if model.__dict__.get("w_spl", None) is not None:
            ...
            ax_sRF_min.imshow(
                gaussian_filter(sRFs_min[i].T, sigma=1),
                cmap=plt.cm.bwr,
                vmax=vmax,
                vmin=-vmax,
            )
        ...
        if model.__dict__.get("w_spl", None) is not None:
            ...
            ax_sRF_max.imshow(
                gaussian_filter(sRFs_max[i].T, sigma=1),
                cmap=plt.cm.bwr,
                vmax=vmax,
                vmin=-vmax,
            )
        ...
        if model.__dict__.get("nl_params_opt", None) is not None:
            ...
            ax_nl.plot(
                xrng, model.nl_basis * model.nl_params_opt[i], color="grey", alpha=0.5
            )
    ...
    if (model.__dict__.get("h_opt", None) is not None) and (
        model.__dict__.get("fnl_fitted", None) is None
    ):
    ...
    elif (model.__dict__.get("h_opt", None) is None) and (
        model.__dict__.get("fnl_fitted", None) is not None
    ):
        if model.output_nonlinearity == "spline" or model.output_nonlinearity == "nn":
            ...
            ax_nl.plot(xrng, nl)
            ax_nl.plot(
                xrng, model.nl_basis * model.nl_params_opt[-1], color="grey", alpha=0.5
            )
    ...
    elif (model.__dict__.get("h_opt", None) is not None) and (
        model.__dict__.get("fnl_fitted", None) is not None
    ):
        dims_h = len(model.h_opt)
        t_hRF = np.linspace(-(dims_h + 1) * dt, -1 * dt, dims_h + 1)[1:]
        ...
        if model.__dict__.get("nl_params_opt", None) is not None:
            if (
                model.output_nonlinearity == "spline"
                or model.output_nonlinearity == "nn"
            ):
                nl_opt = model.fnl_fitted(model.nl_params_opt[-1], model.nl_xrange)
                ax_nl.plot(xrng, nl_opt, color="black", linewidth=3)
                ax_nl.plot(
                    xrng,
                    model.nl_basis * model.nl_params_opt[-1],
                    color="grey",
                    alpha=0.5,
                )
            else:
    ...
    if response_type == "spike":
        markerline, stemlines, baseline = ax_pred.stem(
            t_pred * dt,
            y_test[t_pred],
            linefmt="black",
            markerfmt="none",
        ...
        ax_pred.plot(
            t_pred * dt, y_test[t_pred], color="black", label=f"{response_type}"
        )

    ax_pred.plot(
        t_pred * dt, y_pred[t_pred], color="C3", linewidth=3, label=f"{pred_score:.3f}"
    )
...
def plot3dn(
    ...
    for i, name in enumerate(ws):
        if "stimulus" in name:
            ...
            stats[name]["tRF_time_diff"].append(
                np.abs(t_tRF[max_coord[0]] - t_tRF[min_coord[0]])
            )
            stats[name]["tRF_activation_diff"].append(
                np.abs(tRF_max[max_coord[0]] - tRF_min[min_coord[0]])
            )
