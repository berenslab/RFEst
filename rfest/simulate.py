...
def gaussian1d(dim=200, std=15.0):
    return uvec(scipy.signal.gaussian(dim, std=std))


def gaussian2d(dims=(25, 25), std=(2.0, 2.0)):
...
def mexicanhat1d(dims=200, std=15.0, a=0.8):
...
def mexicanhat2d(dims=(25, 25), std=(3.0, 3.0), a=0.55):
...
def gabor2d(dims=(25, 25), omega=0.5, theta=np.pi / 6, func=np.cos, K=1.0):
...
def V1complex_2d(dims=(30, 40), scale=(0.025, 0.03)):
...
def flickerfield(
    ...
    if beta is not None:
        X = colornoise1d(
            1, dims=n_samples, beta=beta, phi=X.flatten(), random_seed=2046
        )[:, np.newaxis]
        X = (X - X.mean()) / X.std()
...
def flickerbar(
    ...
    if beta is not None:
        ...
        X = colornoise1d(
            n_samples=n_samples, dims=nx, beta=beta, phi=X, random_seed=random_seed
        )
        X = (X - X.mean()) / X.std()
...
def noise2d(
    ...
    if beta is not None:
        ...
        X = colornoise2d(
            n_samples=n_samples, dims=dims, beta=beta, phi=X, random_seed=random_seed
        )
        X = (X - X.mean()) / X.std()
