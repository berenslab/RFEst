...
def test_glm_3d_split_data_test():
    ...
    df = tuple([int(np.maximum(dim / 2., 3)) for dim in dims])
    ...
    model.fit(
        y={'train': y_train, 'dev': y_dev}, num_iters=300, verbose=0, step_size=0.03, beta=0.001, metric='corrcoef')
...
def test_glm_3d_subunits():
    ...
    model.fit(
        y={'train': y_train, 'dev': y_dev}, num_iters=300, verbose=100, step_size=0.03, beta=0.001, metric='corrcoef')

    assert model.score(X_train, y_train, metric='corrcoef') > 0.4
    assert model.score(X_dev, y_dev, metric='corrcoef') > 0.4
