...
def _fit_glm(
    ...
    if Xy_dev is not None:
        model.add_design_matrix(
            Xy_dev[0], dims=dims, df=df, name="stimulus", kind="dev"
        )
    ...
    if fit_history_filter:
        ...
        if Xy_dev is not None:
            model.add_design_matrix(
                Xy_dev[1], dims=[5], df=[3], kind="dev", name="history"
            )
    ...
    model.fit(
        y=fit_y, num_iters=300, verbose=0, step_size=0.03, beta=0.001, metric="mse"
    )
    return model
...
def test_glm_3d_stim():
    ...
    (X_train, y_train), (_, _), (_, _) = split_data(
        X, y, dt, frac_train=1.0, frac_dev=0.0
    )
    model = _fit_glm(
        Xy_train=(X_train, y_train), dims=dims, dt=dt, fit_history_filter=False
    )
...
def test_glm_3d_stim_train_dev_test():
    ...
    (X_train, y_train), (X_dev, y_dev), (X_test, y_test) = split_data(
        X, y, dt, frac_train=0.6, frac_dev=0.2
    )
    model = _fit_glm(
...
def test_glm_3d_stim_2subunits():
    ...
    assert (
        mse(
            uvec(model.w["opt"]["stimulus_s0"] + model.w["opt"]["stimulus_s1"]),
            uvec(w_true.flatten()),
        )
        < 0.01
    )
...
def test_glm_3d_stim_history():
    ...
    assert (
        model.score(
            {"stimulus": X_train, "history": y_train}, y_train, metric="corrcoef"
        )
        > 0.4
    )
    assert (
        model.score({"stimulus": X_dev, "history": y_dev}, y_dev, metric="corrcoef")
        > 0.3
    )
    assert (
        model.score({"stimulus": X_test, "history": y_test}, y_test, metric="corrcoef")
        > 0.2
    )
    assert mse(uvec(model.w["opt"]["stimulus"]), uvec(w_true.flatten())) < 0.01
