...
def _fit_w_lnln(X, y, dims, dt, num_subunits=1, Xy_dev=None):
...
def test_lnln_2d_stim():
    w_true, X, y, dt, dims = generate_data_2d_stim(
        noise="white", rf_kind="complex_small", y_distr="none"
    )
    w_opt = _fit_w_lnln(X, y, dims, dt)
    assert mse(uvec(w_opt.flatten()), uvec(w_true.flatten())) < 0.01


def test_lnln_2d_stim_spikes():
    w_true, X, y, dt, dims = generate_data_2d_stim(
...
def test_lnln_2d_stim_2subunits():
    w_true, X, y, dt, dims = generate_data_2d_stim(
        noise="white", rf_kind="complex_small", y_distr="none"
    )
    w_opt = _fit_w_lnln(X, y, dims, dt, num_subunits=2)
    assert w_opt.size == w_true.size * 2


def test_lnln_3d_stim_2subunits():
    w_true, X, y, dt, dims = generate_data_3d_stim(
        noise="white", rf_kind="complex_small", y_distr="none"
    )
    w_opt = _fit_w_lnln(X, y, dims, dt, num_subunits=2)
...
def test_lnln_2d_stim_split():
    ...
    w_opt = _fit_w_lnln(X_train, y_train, dims, dt, Xy_dev=(X_dev, y_dev))
    assert mse(uvec(w_opt.flatten()), uvec(w_true.flatten())) < 0.01
